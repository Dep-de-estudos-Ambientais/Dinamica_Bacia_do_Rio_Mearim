...
const BASEMAPS = {
  satellite: {
    ...
    layers: [
      {id: 'esri-world-imagery', type: 'raster', source: 'esriWorldImagery'}
    ]
  },
  ...
  osm: {
    ...
    layers: [
      {id: 'osm-standard', type: 'raster', source: 'osmStandard'}
    ]
  }
};
...
const map = new maplibregl.Map({
  container: 'map', style: BASEMAPS.satellite, center: [-45.9, -5.2], zoom: 5.7, attributionControl: true
});
...
document.getElementById('basemapSelect').addEventListener('change',e=>{setLoading(true,'Trocando mapa-base…');map.setStyle(BASEMAPS[e.target.value]);map.once('style.load',async()=>{await rehydrate();applyOpacity();raiseReferenceLayers();setLoading(false);});});
